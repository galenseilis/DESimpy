...
class School:
    ...
    def __init__(self, env: EventScheduler) -> None:
        ...
        self.pupil_procs = [self.env.schedule(Event(self.env.current_time, lambda i=i: self.pupil(i))) for i in range(3)]
    ...
    def pupil(self, pupil_id: int) -> None:
        ...
        for _ in range(2):
            print(f"Pupil {pupil_id}: Waiting for the class to end at time {self.env.current_time}")
            
            # Wait until the class has ended (when bell rings)
            while not self.class_ended:
                yield self.env.schedule(Event(self.env.current_time + 0.1, lambda: None))  # Small delay

            print(f"Pupil {pupil_id}: Class ended, responding at time {self.env.current_time}")
