...
def test_activate_next_event_by_condition_single_event_true():
    ...
    assert event.status == EventStatus.ACTIVE, "The event should be activated as it meets the condition."
...
def test_activate_next_event_by_condition_single_event_false():
    ...
    assert event.status == EventStatus.INACTIVE, "The event should remain inactive as it does not meet the condition."
...
def test_activate_next_event_by_condition_multiple_events_first_meets_condition():
    ...
    assert event1.status == EventStatus.ACTIVE, "The first event should be activated as it meets the condition."
    assert event2.status == EventStatus.INACTIVE, "The second event should remain inactive as it does not meet the condition."
...
def test_activate_next_event_by_condition_multiple_events_second_meets_condition():
    ...
    assert event1.status == EventStatus.INACTIVE, "The first event should remain inactive as it does not meet the condition."
    assert event2.status == EventStatus.ACTIVE, "The second event should be activated as it meets the condition."
...
def test_activate_next_event_by_condition_first_match_only():
    ...
    assert event1.status == EventStatus.ACTIVE, "The first event matching the condition should be activated."
    assert event2.status == EventStatus.INACTIVE, "The second event with the same time should remain inactive."

def test_activate_next_event_by_condition_empty_queue():
    """Test `activate_next_event_by_condition` when the event queue is empty."""
    scheduler = EventScheduler()
    
    scheduler.activate_next_event_by_condition(lambda s, e: e.time == 1)  # No events in the queue
    
    assert scheduler.event_queue == [], "Queue should remain empty when no events are scheduled."
...
def test_activate_next_event_by_condition_multiple_conditions():
    ...
    scheduler.activate_next_event_by_condition(lambda s, e: e.context.get('type') == 'A' and e.time == 3)

    assert event1.status == EventStatus.INACTIVE, "The first event should remain inactive as it does not meet all conditions."
    assert event2.status == EventStatus.INACTIVE, "The second event should remain inactive as it does not meet the conditions."
    assert event3.status == EventStatus.ACTIVE, "The third event should be activated as it meets all conditions."
...
def test_activate_next_event_by_condition_with_no_match_in_complex_condition():
    ...
    scheduler.activate_next_event_by_condition(lambda s, e: e.context.get('type') == 'C')
