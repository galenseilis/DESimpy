...
def test_peek_multiple_events_ordered(scheduler: EventScheduler, event: Callable[[float], Event]):
...
def test_peek_multiple_events_unordered(scheduler: EventScheduler, event: Callable[[float], Event]):
...
def test_peek_after_event_removal(scheduler: EventScheduler, event: Callable[[float], Event]):
...
def test_peek_prescheduling_negative_time(scheduler: EventScheduler, event: Callable[[float], Event]):
    """Test that peek allows negative event times if prescheduled."""
    event1 = event(-5.0)
    scheduler.schedule(event1)
    assert scheduler.peek() == -5.0

def test_peek_after_all_events_canceled(scheduler: EventScheduler, event: Callable[[float], Event]):
