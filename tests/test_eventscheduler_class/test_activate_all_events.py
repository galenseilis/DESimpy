...
def test_activate_all_events_with_context_conditions():
    ...
    assert event1.status == EventStatus.ACTIVE, "The first event with high priority should be activated."
    assert event2.status == EventStatus.ACTIVE, "The low priority event should be activated."
    assert event3.status == EventStatus.ACTIVE, "The second high priority event should be activated."
...
def test_activate_all_events_with_already_scheduled_and_unscheduled_events():
    ...
    assert event1.status == EventStatus.ACTIVE, "The scheduled event should be activated."
    assert event2.status == EventStatus.INACTIVE, "The unscheduled event should remain inactive."
...
def test_activate_all_events_empty_context():
    ...
    assert event.status == EventStatus.ACTIVE, "Event with empty context should still be activated."
