...
def test_cancel_next_event_by_condition_no_matching_event():
    ...
    assert (event1.time, event1) in scheduler.event_queue, "Event1 should remain in the queue."
...
def test_cancel_next_event_by_condition_multiple_matching_events():
    ...
    assert ((event1.time, event1) not in scheduler.event_queue and (event2.time, event2) in scheduler.event_queue) or ((event1.time, event1) in scheduler.event_queue and (event2.time, event2) not in scheduler.event_queue), \
