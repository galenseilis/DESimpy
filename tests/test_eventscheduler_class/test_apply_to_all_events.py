...
def test_apply_to_all_events_empty_queue(scheduler: EventScheduler):
    ...
    def increment_time(event: Event) -> None:
        event.time += 5  # Arbitrary function that would alter event time if events existed
...
def test_apply_to_all_events_increment_time(scheduler: EventScheduler, event: Callable[[float], Event]):
...
def test_apply_to_all_events_double_time(scheduler: EventScheduler, event: Callable[[float], Event]):
...
def test_apply_to_all_events_custom_attribute_change(scheduler: EventScheduler, event: Callable[[float], Event]):
    ...
    def toggle_active(event: Event) -> None:
        event.status = EventStatus.ACTIVE if event.status == EventStatus.INACTIVE else EventStatus.INACTIVE
...
def test_apply_to_all_events_no_change(scheduler: EventScheduler, event: Callable[[float], Event]):
...
def test_apply_to_all_events_negative_time(scheduler: EventScheduler, event: Callable[[float], Event]):
