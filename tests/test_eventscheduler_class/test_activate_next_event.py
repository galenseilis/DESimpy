...
@pytest.fixture
def event_factory() -> Callable[[float, Callable[[], Any], dict[Any, Any] | None], Event]:
    """Factory fixture to create events at a specified time with an optional action."""
    def _create_event(time: float, action: Callable[[], Any] | None = None, context: dict[Any, Any] | None = None):
        return Event(time=time, action=action, context=context)
    return _create_event

EVENT_FACTORY_TYPE = Callable[[], Callable[[float, Callable[[], Any], dict[Any, Any] | None], Event]]
...
def test_activate_next_event_multiple_events(scheduler: EventScheduler):
    ...
    assert event1.status == EventStatus.ACTIVE, "The earliest event should be activated."
    assert event2.status == EventStatus.INACTIVE, "The later event should remain inactive."
...
def test_activate_next_event_with_active_event(scheduler: EventScheduler):
    ...
    assert event.status == EventStatus.ACTIVE, "Event should remain active after reactivation."
...
def test_activate_next_event_does_not_alter_later_events(scheduler: EventScheduler):
    ...
    assert event2.status == EventStatus.INACTIVE, "The second event should remain inactive."
...
def test_activate_next_event_only_affects_next(scheduler: EventScheduler):
    ...
    assert event1.status == EventStatus.ACTIVE, "Only the earliest event should be activated."
    assert event2.status == EventStatus.INACTIVE, "Subsequent events should remain inactive."
