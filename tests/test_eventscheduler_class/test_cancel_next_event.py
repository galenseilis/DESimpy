...
def test_cancel_next_event_single_event():
    ...
    assert len(scheduler.event_queue) == 0, "The event queue should be empty after cancellation."
...
def test_cancel_next_event_multiple_events():
    ...
    assert len(scheduler.event_queue) == 2, "The event queue should have two events remaining."
    assert (event2.time, event2) in scheduler.event_queue, "Event2 should remain in the queue."
    assert (event3.time, event3) in scheduler.event_queue, "Event3 should remain in the queue."
...
def test_cancel_next_event_no_events_remaining():
    ...
    assert len(scheduler.event_queue) == 0, "The event queue should be empty after cancellation of all events."
...
def test_cancel_next_event_queue_ordering():
    ...
    assert (event1.time, event1) not in scheduler.event_queue, "Event1 should be cancelled and removed from the queue."
...
def test_cancel_next_event_already_cancelled_event():
    ...
    assert len(scheduler.event_queue) == 0, "The event queue should be empty after cancelling both events."
...
def test_cancel_next_event_event_with_multiple_actions():
    ...
    assert len(scheduler.event_queue) == 1, "The event queue should have one event remaining."
    assert (event1.time, event1) not in scheduler.event_queue, "Event1 should be cancelled and removed from the queue."
    assert (event2.time, event2) in scheduler.event_queue, "Event2 should remain active."
...
def test_cancel_next_event_reschedules():
    ...
    assert len(scheduler.event_queue) == 2, "The event queue should have two events."
    assert (event2.time, event2) in scheduler.event_queue, "Event2 should be the only event in the queue."
