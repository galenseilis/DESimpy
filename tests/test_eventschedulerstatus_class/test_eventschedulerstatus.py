...
def test_event_scheduler_status_members():
    ...
    assert hasattr(EventSchedulerStatus, "INACTIVE"), "INACTIVE should be a member of EventSchedulerStatus."
    assert hasattr(EventSchedulerStatus, "ACTIVE"), "ACTIVE should be a member of EventSchedulerStatus."

def test_event_scheduler_status_auto_values():
    """Verify the auto-assigned values for the EventSchedulerStatus enum."""
    assert EventSchedulerStatus.INACTIVE.value == 1, "INACTIVE should have a value of 1."
...
def test_event_scheduler_status_distinction():
    ...
    assert EventSchedulerStatus.INACTIVE != EventSchedulerStatus.ACTIVE, "INACTIVE and ACTIVE should be distinct."
...
def test_event_scheduler_status_names():
    ...
    assert EventSchedulerStatus.INACTIVE.name == "INACTIVE", "The name of INACTIVE should be 'INACTIVE'."
    assert EventSchedulerStatus.ACTIVE.name == "ACTIVE", "The name of ACTIVE should be 'ACTIVE'."
...
def test_event_scheduler_status_str_representation():
    ...
    assert str(EventSchedulerStatus.INACTIVE) == "EventSchedulerStatus.INACTIVE", (
        "String representation of INACTIVE is incorrect."
    )
    assert str(EventSchedulerStatus.ACTIVE) == "EventSchedulerStatus.ACTIVE", (
