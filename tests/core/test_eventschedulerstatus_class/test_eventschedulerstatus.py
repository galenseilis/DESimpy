...
def test_event_scheduler_status_auto_values():
    ...
    assert EventSchedulerStatus.INACTIVE.value == "inactive", (
        "INACTIVE should have a value of inactive."
    )
    assert EventSchedulerStatus.ACTIVE.value == "active", (
...
def test_event_scheduler_status_distinction():
    ...
    assert EventSchedulerStatus.INACTIVE != EventSchedulerStatus.ACTIVE, (
...
def test_event_scheduler_status_names():
    ...
    assert EventSchedulerStatus.INACTIVE.name == "INACTIVE", (
        "The name of INACTIVE should be 'INACTIVE'."
    )
    assert EventSchedulerStatus.ACTIVE.name == "ACTIVE", (
...
def test_event_scheduler_status_str_representation():
    ...
    assert str(EventSchedulerStatus.INACTIVE) == "inactive", (
        "String representation of INACTIVE is incorrect."
    )
    assert str(EventSchedulerStatus.ACTIVE) == "active", (
