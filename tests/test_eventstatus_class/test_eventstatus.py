...
def test_event_status_members():
    ...
    assert hasattr(EventStatus, "INACTIVE"), "INACTIVE should be a member of EventStatus."
...
def test_event_status_comparisons():
    ...
    assert EventStatus.INACTIVE != EventStatus.ACTIVE, "INACTIVE and ACTIVE should not be equal."
    assert EventStatus.INACTIVE.value < EventStatus.ACTIVE.value, (
...
def test_event_status_string_representation():
    ...
    assert str(EventStatus.INACTIVE) == "EventStatus.INACTIVE", (
        "String representation of INACTIVE is incorrect."
    )
    assert str(EventStatus.ACTIVE) == "EventStatus.ACTIVE", (
